...
// 2. THÊM MỚI (Lưu ý: SQL của bạn không có MaCanHo nên tôi đã bỏ)
const createTamTru = async (req, res) => {
    try {
        // Nhận tên biến từ Frontend (Lưu ý Frontend cần gửi đúng key này)
...
// 3. CẬP NHẬT (Dựa theo ID)
const updateTamTru = async (req, res) => {
    try {
        const { id } = req.params; // ID là số int
