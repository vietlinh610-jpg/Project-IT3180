...
const createTamVang = async (req, res) => {
    try {
        ...
        const checkNK = await pool.request()
        ...
        await pool.request()
            ...
            .input('NgayVe', sql.Date, NgayVe) // Cột mới
...
const deleteTamVang = async (req, res) => {
    try {
        const { id } = req.params;
        const pool = await connectDB();
        await pool.request().input('ID', sql.Int, id).query("DELETE FROM tam_vang WHERE ID = @ID");
...
module.exports = { getListTamVang, createTamVang, updateTamVang, deleteTamVang };