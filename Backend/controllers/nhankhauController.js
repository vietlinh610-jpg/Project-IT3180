...
const getNhanKhauByCanHo = async (req, res) => {
    try {
        ...
        if (chCheck.recordset.length === 0) return res.status(404).json({ message: "Không tìm thấy căn hộ" });
...
const createNhanKhau = async (req, res) => {
    try {
        const { 
            MaNhanKhau, MaHoKhau, HoTen, GioiTinh, 
            NgaySinh, DanToc, TonGiao, QuocTich, 
            NgheNghiep, SoCCCD, NoiSinh, QuanHeVoiChuHo 
        } = req.body;
        ...
        if (err.number === 2627) {
            return res.status(409).json({ message: "Mã nhân khẩu hoặc số CCCD đã tồn tại!" });
        }
...
const getNhanKhauByMa = async (req, res) => {
    try {
        const { id } = req.params;
        const maCanTim = id.trim(); // Xóa khoảng trắng thừa tránh lỗi ngớ ngẩn
        ...
        if (result.recordset.length === 0) {
            return res.status(404).json({ message: "Không tìm thấy nhân khẩu trong hệ thống" });
        }
        ...
        const responseData = {
            ...
            DanhSachCanHo: listCanHo // Frontend sẽ dùng cái này để tạo Dropdown
        };
